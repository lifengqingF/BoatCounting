...
using namespace std;
using namespace cv;
int ObjectDetection::detect_waterline(Mat& image1, Mat& image2)
{
...
int ObjectDetection::detectwaterline(Mat& image)
{
...
int ObjectDetection::erode(Mat & image, Mat& dst)
{
...
int ObjectDetection::dilate(Mat & image, Mat& dst)
{
...
int ObjectDetection::cropcontour(Mat& src_image, Mat& src_gray, Mat&dst_poly, Mat&dst_hull, int pn)
{
	Mat canny_output;
...
int ObjectDetection::DetectObjects(Mat& image1, Mat& image2)
{

	Mat addup = image2 - image1;
	Mat deduce = image1 - image2;

	Mat Difference = addup + addup;
	DetectObjects(Difference);
...
int ObjectDetection::DetectObjects(Mat& image)
{
	Mat image_threshold_smooth;
	::threshold(image, image_threshold_smooth, 30, 30, CV_THRESH_TOZERO); //
	Mat crop_contour_poly;
	Mat crop_contour_hull;
	cropcontour(image, image_threshold_smooth, crop_contour_poly, crop_contour_hull, 1);

	if (visualize) {
		imshow("addup_threshold", image_threshold_smooth);
		imshow("crop_addup_contour_poly", crop_contour_poly);
	}
	return 0;
}
int ObjectDetection::sort_objects()//if we saved countour ,no need to check if rectangles will overlapped
{
...
int ObjectDetection::drawit(Mat& showme, int boatcount)
{
#define LINEW 1
...
vector<Object>::iterator ObjectDetection::checkthis(vector<Object>::iterator i)
{
